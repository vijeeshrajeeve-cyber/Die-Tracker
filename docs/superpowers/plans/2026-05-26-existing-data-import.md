# Existing Data Import UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a self-contained React component for importing existing die details and production data by plant, surfaced as a new admin-only section in Settings.

**Architecture:** New `src/pages/ExistingDataPage.jsx` with all state local to the component. Integrated into `src/pages/SettingsPage.jsx` after the Profile Master section. No changes to `DieOrderingSystem.jsx` — `plants`, `theme`, and `setToast` already flow into SettingsPage.
...
| File | Change |
|------|--------|
| `src/pages/ExistingDataPage.jsx` | **Create** — full component |
| `src/pages/SettingsPage.jsx` | **Modify** — add import + render ExistingDataPage after Profile Master |

---

### Task 1: Create ExistingDataPage.jsx

**Files:**
- Create: `src/pages/ExistingDataPage.jsx`
    ...
    <div style={{ padding: '1.25rem 0 0.5rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
        ...
        <div>
          ...
          {fileName && (
            <span style={{ marginLeft: '10px', fontSize: '0.8rem', color: theme.textDim }}>{fileName}</span>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ gridColumn: 'span 2', background: theme.cardBg, borderRadius: '16px', padding: '1.5rem', border: `1px solid ${theme.cardBorder}` }}>
      ...
      <div style={{ marginTop: '1.5rem', borderTop: `1px solid ${theme.cardBorder}`, paddingTop: '1rem' }}>
        <h4 style={{ fontSize: '0.875rem', fontWeight: 600, color: theme.text, marginBottom: '0.75rem', margin: '0 0 0.75rem' }}>
...
```bash
git add src/pages/ExistingDataPage.jsx
git commit -m "feat: add ExistingDataPage component for existing data import"
```

---

### Task 2: Integrate ExistingDataPage into SettingsPage

**Files:**
- Modify: `src/pages/SettingsPage.jsx`

The Profile Master section ends at approximately line 249 (`</div>`). The Budget Targets section begins immediately after. Insert ExistingDataPage between them.

- [ ] **Step 1: Add the import at the top of SettingsPage.jsx**
...
- [ ] **Step 2: Insert the ExistingDataPage section after Profile Master**

Find this line in `src/pages/SettingsPage.jsx` (the closing `</div>` of the Profile Master card, immediately before the Budget Targets comment):
...
- [ ] **Step 4: Commit all remaining uncommitted changes together with this integration**

```bash
git add src/pages/SettingsPage.jsx src/pages/ExistingDataPage.jsx \
        init.sql server/db.cjs server/index.cjs \
        server/routes/backup-requests.cjs server/routes/existing-data.cjs \
        server/services/dieOrderTemplate.cjs \
        src/api.js src/components/backup/BackupDieRequests.jsx \
        package.json package-lock.json
git commit -m "feat: existing data import UI and backup die order PDF generation"
```
