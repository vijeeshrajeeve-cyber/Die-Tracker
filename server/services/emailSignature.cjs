'use strict';
// The company's standard email signature — server-side twin of
// src/utils/emailSignature.js.
//
...
// sending (frontend), while reminders and approval notices are sent without a
// human in the loop (backend). The backend image ships only server/, so it
// cannot import the frontend module. emailSignature.test.cjs renders both and
...
const dieDesignSignature = () => buildSignature(DIE_DESIGN);
const dieDesignSignatureText = () => signatureText(DIE_DESIGN);
...
const userSignature = (user) => buildSignature(asUser(user));
const userSignatureText = (user) => signatureText(asUser(user));

module.exports = {
  DIE_DESIGN, buildSignature, signatureText,
  dieDesignSignature, dieDesignSignatureText,
  userSignature, userSignatureText,
};
